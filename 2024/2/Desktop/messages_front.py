...
def exibir_popup(titulo, mensagem):
    # Criar a janela do popup como Toplevel ao invés de Tk
    janela = Toplevel()
    janela.title("Atenção!")
   
    # Torna o popup modal (bloqueia interação com a janela principal)
    janela.grab_set()
   
    # Centraliza o popup em relação à tela
    window_width = 800
    window_height = 400
    ...
    # Botão fechar
    btn_fechar = Button(janela, text="Fechar", bg="#FFFFFF", fg="#331c8f", width=25, height=1, font=("Montserrat", 12, "bold"), command=janela.destroy)
